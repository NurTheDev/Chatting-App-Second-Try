...
import {MdLogout, MdOutlineCloudUpload, MdOutlineMessage,} from "react-icons/md";
...
const Sidebar = () => {
        ...
        window.cloudinary.openUploadWidget({
            ...
            if (!error && result && result.event === "success") {
                ...
                update(userRef, {
                    ...
                    update(ref(db, `FriendList/${auth.currentUser.uid}`), {
                        img: result.info.secure_url
                    }).then(() => {
                        update(ref(db, `FriendRequest/${auth.currentUser.uid}`), {
                            img: result.info.secure_url
                        }).then(() => {
                            console.log("Avatar updated successfully");
                        }).catch((error) => {
                            console.error("Error updating avatar in FriendRequest:", error);
                        });
                    }).catch((error) => {
                        console.error("Error updating avatar in FriendList:", error);
                    })
                });
            }
        })
    ...
    return (
        <div className="grid grid-cols-12 py-9 px-8 h-screen w-full">
            <div
                className="col-span-1  bg-primary-purple rounded-[20px] py-10 flex flex-col items-center justify-between">
